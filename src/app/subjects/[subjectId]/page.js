...
import { apiUploadPdf, apiUploadVideo, apiUploadText } from "@/utils/api";
...
/* ═══════════════════════════════════════════════════════════════════
   ADD LECTURE MODAL — creates lecture then uploads content
══════════════════════════════════════════════════════════════════════ */
function AddLectureModal({ open, onClose, onAdd }) {
  ...
  const [phase,      setPhase]      = useState(""); // "creating" | "uploading" | ""

  const reset = () => {
    setName(""); setVideo(""); setFile(null); setError("");
    setDragging(false); setUploadPct(0); setPhase("");
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Step 1 — create the lecture record
      ...
      if (lectureId) {
        ...
        if (file) {
          // PDF / document upload
          await apiUploadPdf(lectureId, file, (pct) => setUploadPct(pct));
        } else if (video.trim()) {
          // YouTube / video URL
          await apiUploadVideo(lectureId, { url: video.trim(), extract_frames: false });
        }
      }
      ...
      setSubmitting(false); setPhase("");
    }
  };

  const phaseLabel = phase === "creating" ? "Creating lecture…" : phase === "uploading" ? `Uploading${uploadPct ? ` ${uploadPct}%` : "…"}` : "Adding…";
          ...
          <motion.div initial={{ opacity: 0, scale: 0.94, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.94, y: 16 }}
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 dark:border-white/8 tc sticky top-0 bg-white dark:bg-[#111827] z-10">
              <div>
                ...
                <p className="text-[12px] text-slate-500 dark:text-gray-400 mt-0.5">Upload a file or add a video link</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[12px] font-bold text-slate-700 dark:text-gray-300 flex items-center gap-1.5 tc">
                  ...
                  <span className="text-slate-400 dark:text-gray-600 font-normal">(YouTube URL)</span>
                </label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-gray-500"><LinkIcon size={14} /></span>
                  <input type="url" value={video}
                    onChange={e => { setVideo(e.target.value); setError(""); }}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[12px] font-bold text-slate-700 dark:text-gray-300 flex items-center gap-1.5 tc">
                  ...
                  <span className="text-slate-400 dark:text-gray-600 font-normal">(PDF / Doc)</span>
                </label>
                {file ? (
                  <div className="flex items-center gap-3 px-4 py-3 rounded-xl tc bg-indigo-50 dark:bg-indigo-500/15 border border-indigo-200 dark:border-indigo-500/30">
                    ...
                    <button type="button" onClick={() => setFile(null)} disabled={submitting}
                      className="border-0 bg-transparent cursor-pointer text-slate-400 dark:text-gray-500 hover:text-red-500 tc p-0">
                      <Trash2 size={15} />
                    </button>
                  </div>
                ) : (
                  <div
                    ...
                    onDrop={handleDrop}
                    onClick={() => !video.trim() && fileRef.current?.click()}
                    className={`flex flex-col items-center justify-center gap-2 py-7 rounded-xl cursor-pointer border-2 border-dashed tc transition-colors
                      ${video.trim() ? "opacity-40 cursor-not-allowed border-slate-200 dark:border-white/8" :
                        dragging ? "border-indigo-400 bg-indigo-50 dark:bg-indigo-500/15"
                    ...
                    <div className="text-[13px] font-semibold text-slate-600 dark:text-gray-400 tc">
                      {video.trim() ? "Remove video link to upload a file" : <>Drop file here or <span className="text-indigo-600 dark:text-indigo-400">browse</span></>}
                    </div>
                    <div className="text-[11px] text-slate-400 dark:text-gray-600 tc">PDF, DOCX, TXT, MP4</div>
                    <input ref={fileRef} type="file" className="hidden"
                      accept=".pdf,.mp4,.webm,.txt,.docx"
...
function SubjectDetailContent() {
  ...
  /* Navigate to chat with lecture metadata in URL */
      ...
      <header className="sticky top-0 z-50 tc bg-white/95 dark:bg-[#0b1020]/92 backdrop-blur-xl border-b border-slate-200 dark:border-white/8 shadow-sm dark:shadow-none">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 flex items-center justify-between gap-3 py-3">
          <div className="flex items-center gap-2 sm:gap-3">
            <motion.button whileHover={{ x: -2 }} whileTap={{ scale: 0.90 }} onClick={() => router.push("/subjects")} className={iconBtn}>
              <ArrowLeft size={17} />
            </motion.button>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <motion.button whileHover={{ y: -1 }} whileTap={{ scale: 0.90 }} onClick={toggleTheme} className={iconBtn}>
              {dark ? <Sun size={16} /> : <Moon size={16} />}
            </motion.button>
            <motion.button whileHover={{ y: -1 }} whileTap={{ scale: 0.90 }} onClick={logout} className={`${iconBtn} hover:!text-red-500 dark:hover:!text-red-400`}>
              <LogOut size={16} />
            </motion.button>
          </div>
        </div>
        ...
        {!loading && !error && (
          <motion.section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4" variants={stagger} initial="hidden" animate="show">
            {lectures.map((l, idx) => {
              ...
              return (
                <motion.button key={l.id ?? l._id} variants={cardAnim}
                  whileHover={isClickable ? { y: -3, boxShadow: "0 16px 40px rgba(2,6,23,0.10)" } : {}}
                  whileTap={isClickable ? { scale: 0.98 } : {}}
                  onClick={() => isClickable && openChat(l)}
                  disabled={!isClickable}
                  className={`text-left w-full rounded-2xl tc bg-white dark:bg-[#111827] border border-slate-200 dark:border-white/8 shadow-[0_2px_14px_rgba(2,6,23,0.05)] dark:shadow-none overflow-hidden cursor-pointer ${!isClickable ? "opacity-70 cursor-wait" : ""}`}>
                  {/* Coloured top band */}
                  ...
                  <div className="px-4 sm:px-5 py-2.5 sm:py-3 flex items-center gap-3 tc">
                    ...
                    {l.description && !l.description.startsWith("http") && (
                      <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold tc text-slate-500 dark:text-gray-400 truncate">
                        <FileText size={11} />{l.description}
                      </span>
                    )}
