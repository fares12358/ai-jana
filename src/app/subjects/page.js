...
import {
  Brain, Moon, Sun, LogOut, Plus, Folder,
  X, BookOpen, AlignLeft, RefreshCw, Trash2,
} from "lucide-react";
          ...
          <motion.div
            ...
            <form onSubmit={handleSubmit} className="px-5 sm:px-6 py-4 sm:py-5 flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-[12px] font-bold text-slate-700 dark:text-gray-300 flex items-center gap-1.5">
                  <BookOpen size={13} className="text-indigo-500" />Subject Name <span className="text-red-500">*</span>
                </label>
                <input autoFocus value={name} onChange={e => { setName(e.target.value); setError(""); }}
                  placeholder="e.g. Machine Learning" disabled={submitting}
                ...
                <AnimatePresence>
                  {error && <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-[12px] text-red-500 font-semibold">{error}</motion.p>}
                </AnimatePresence>
              </div>

              <div className="flex flex-col gap-1.5">
                <label className="text-[12px] font-bold text-slate-700 dark:text-gray-300 flex items-center gap-1.5">
                  <AlignLeft size={13} className="text-indigo-500" />Description
              ...
              <div className="flex gap-3 pt-1 pb-1 sm:pb-0">
                ...
                <motion.button whileTap={{ scale: 0.97 }} type="submit" disabled={submitting}
                  ...
                  {submitting
                    ? <span className="flex items-center justify-center gap-2"><span className="w-3.5 h-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin" />Creating…</span>
                    : "Create Subject"}
                </motion.button>
              </div>
            </form>
...
function DeleteConfirmModal({ open, subjectName, onConfirm, onCancel, deleting }) {
  return (
    <AnimatePresence>
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div
            ...
            <h3 className="font-black text-[16px] text-slate-900 dark:text-white mb-1">Delete Subject?</h3>
            <p className="text-[13px] text-slate-500 dark:text-slate-400 leading-relaxed mb-5">
              <span className="font-bold text-slate-700 dark:text-slate-200">&ldquo;{subjectName}&rdquo;</span> and all its lectures will be permanently deleted. This cannot be undone.
            </p>
      ...
      <header className="sticky top-0 z-50 tc bg-white/95 dark:bg-[#0b1020]/92 backdrop-blur-xl border-b border-slate-200 dark:border-white/8 shadow-sm dark:shadow-none">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 flex items-center justify-between gap-3 py-3">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <button onClick={toggleTheme} className={iconBtn}>{dark ? <Sun size={15} /> : <Moon size={15} />}</button>
            <button onClick={logout} className={`${iconBtn} hover:!text-red-500 dark:hover:!text-red-400`}><LogOut size={15} /></button>
          </div>
        </div>
      </header>
              ...
              return (
                <motion.div key={s.id} variants={cardAnim}
                  ...
                  <button
                    ...
                    <div className={`${c.bg} w-11 h-11 sm:w-[52px] sm:h-[52px] rounded-xl grid place-items-center flex-shrink-0 tc`}>
                      <Folder size={18} className={c.icon} />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-[15px] sm:text-[16px] font-black truncate mb-0.5 text-slate-900 dark:text-white tc">{s.name}</div>
                      {s.description && (
                        <div className="text-[12px] text-slate-400 dark:text-gray-500 truncate tc">{s.description}</div>
                      )}
                    </div>
                  </button>
        ...
        {!loading && !error && subjects.length === 0 && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
            ...
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center mx-auto mb-3 sm:mb-4 tc bg-indigo-50 dark:bg-indigo-500/15">
              <Folder size={22} className="text-indigo-500 dark:text-indigo-400" />
            </div>
            ...
            <p className="text-[12px] sm:text-[13px] mb-5 sm:mb-6 text-slate-400 dark:text-gray-500 tc max-w-[28ch] mx-auto">
              Create your first subject to start organizing your lectures.
            </p>
