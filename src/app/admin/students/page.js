 ...
 *
 * Despite the route name, this page shows lecture + subject management data.
 * There is no student-list endpoint in the API; student engagement metrics
 * come from GET /admin/analytics (engagement_count per subject).
 *
 * Data sources:
 *   GET /admin/analytics           → engagement counts per subject
 *   GET /subjects/                 → subject list
 *   GET /lectures/subject/{id}     → lectures per subject
 */
...
import { useAdminData, relTime, GRAD_COLORS } from "@/hooks/useAdminData";
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-[11px] font-bold border
      ${styles[status] ?? "bg-slate-100 dark:bg-white/8 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-white/10"}`}>
...
function IngestionBar({ status }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[12px] font-semibold text-slate-600 dark:text-slate-400 flex-shrink-0">
        {cfg.pct}%
      </span>
    </div>
  );
...
export default function LecturesOverviewPage() {
  ...
  /* Build engagement map from analytics array */
  ...
  /* KPI cards */
  const STATS = [
    { label: "Total Subjects",    value: String(totalSubjects), Icon: FaFolderOpen,  bg: "bg-indigo-50 dark:bg-indigo-500/15",   ic: "text-indigo-600 dark:text-indigo-400"   },
    { label: "Total Lectures",    value: String(totalLectures), Icon: FaBookOpen,    bg: "bg-purple-50 dark:bg-purple-500/15",   ic: "text-purple-600 dark:text-purple-400"   },
    { label: "Ready for AI",      value: String(completedLec),  Icon: FaCheckCircle, bg: "bg-emerald-50 dark:bg-emerald-500/15", ic: "text-emerald-600 dark:text-emerald-400" },
    { label: "Processing",        value: String(processingLec), Icon: FaClock,       bg: "bg-amber-50 dark:bg-amber-500/15",    ic: "text-amber-600 dark:text-amber-400"     },
  ];

  /* Subject cards with engagement */
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] sm:text-[14px] text-slate-500 dark:text-slate-400 mt-0.5">
            Subjects &amp; lectures · engagement from{" "}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        {STATS.map(s => (
          <div key={s.label}
            ...
            <div>
              <p className="text-[11px] sm:text-[12px] text-slate-500 dark:text-slate-400 font-medium leading-tight">
                {s.label}
              </p>
              {loading
            ...
            <div key={s._sid || i}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  ...
                  {s.engagement > 0 && (
                    <span className="text-[11px] font-semibold text-indigo-500 dark:text-indigo-400 flex-shrink-0">
                      {s.engagement} chats
                    </span>
                  )}
                </div>
                {s.description && (
                  <p className="text-[11px] text-slate-400 dark:text-slate-500 truncate mt-0.5">{s.description}</p>
                )}
            ...
            <thead>
              <tr className="border-b border-slate-100 dark:border-white/8 bg-slate-50 dark:bg-white/[0.02]">
                {["Lecture", "Subject", "Ingestion", "Status", "Created"].map(h => (
                  <th key={h}
                    className="text-left px-4 sm:px-6 py-3 text-[10px] sm:text-[11px] font-extrabold uppercase tracking-wide text-slate-500 dark:text-slate-400 whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {loading
                ...
                : filtered.map((l, idx) => (
                    <tr key={l.id ?? l._id ?? idx}
                      className="hover:bg-slate-50 dark:hover:bg-white/[0.025] transition-colors">
                      ...
                      <td className="px-4 sm:px-6 py-3 sm:py-4">
                        <div className="flex items-center gap-1.5">
                          ...
                          {l.status === "failed" && (
                            <FaExclamationCircle className="text-red-400 text-[11px]" />
                          )}
                        </div>
                      </td>
                      <td className="px-4 sm:px-6 py-3 sm:py-4">
                ...
                <tr>
                  <td colSpan={5} className="py-16 text-center">
                    <p className="text-[14px] font-semibold text-slate-400 dark:text-slate-500">
                      {search || subjFilter !== "all" || statFilter !== "all"
